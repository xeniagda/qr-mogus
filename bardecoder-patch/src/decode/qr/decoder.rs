...
impl Decode<QRData, String, QRError> for QRDecoder {
    fn decode(&self, data: Result<QRData, QRError>) -> Result<String, QRError> {
        self.decode_with_error_count(data).map(|r| r.0)
    }
}

impl QRDecoder {
    pub fn decode_with_error_count(&self, data: Result<QRData, QRError>) -> Result<(String, u32), QRError> {
        let qr_data = data?;
